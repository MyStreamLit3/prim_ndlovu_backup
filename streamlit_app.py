...
try:
    ...
    from mediapipe.tasks.python.vision import PoseLandmark as PL
...
# ─── JOINT EXTRACTION (new Tasks API) ────────────────────────────────────────
...
def extract_joint_angles(landmarks):
    ...
    def pt(idx): return lm_xyz(landmarks, idx)
...
def compute_full_rula(angles):
    ...
    sa  = RULA_TABLE_A.get((min(ua, 4), min(la, 2), min(w, 4)), 5) + 1
...
def build_heatmap_df(angles, rula_data, reba_data):
    ...
    return pd.DataFrame([(r, s) for r, s in body_regions.items()], columns=["Region", "Risk Score"])
...
if not MP_AVAILABLE:
    st.error(f"MediaPipe could not be initialised: {_MP_ERROR}")
    st.stop()
...
with tab_live:
    ...
    with alert_col1: rula_ph = st.empty()
    with alert_col2: reba_ph = st.empty()
    with alert_col3: ocra_ph = st.empty()
...
with tab_heatmap:
    ...
    heatmap_ph = st.empty()
    st.caption("Green = low risk | Orange = medium | Red = high risk.")
...
def render_idle():
    ...
    for ph, label in [(rula_ph, "RULA"), (reba_ph, "REBA"), (ocra_ph, "OCRA")]:
        ph.markdown(f'<div class="risk-card card-gray"><b>{label}</b><br>—</div>', unsafe_allow_html=True)
...
if export_btn and st.session_state.history:
    ...
    st.sidebar.download_button("Download CSV", csv_bytes,
...
def draw_landmarks_on_image(rgb_image, landmarks, color=(0, 255, 0)):
    h, w = rgb_image.shape[:2]
    img = rgb_image.copy()
    pts = [(int(lm.x * w), int(lm.y * h)) for lm in landmarks]
    for s, e in CONNECTIONS:
        if s < len(pts) and e < len(pts):
            cv2.line(img, pts[s], pts[e], color, 2)
    for x, y in pts:
...
if video_source is not None and run_analysis:
    ...
    while cap.isOpened():
        ...
        # ── Run pose landmarker (new Tasks API) ──
        mp_image  = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb)
        detection = pose_model.detect(mp_image)
        ...
        if detection.pose_landmarks:
            ...
            # Draw 2-D skeleton
            if show_skeleton_2d:
                rgb = draw_landmarks_on_image(rgb, lms)
            ...
            for s, e in CONNECTIONS:
                if s < len(xs) and e < len(xs):
                    fig3d.add_trace(go.Scatter3d(
                        ...
                        mode="lines",
                        line=dict(color=score_to_color(rula_d.get("rula_grand", 1)), width=3),
                        showlegend=False,
                    ))
        fig3d.update_layout(
        ...
        if rula_d:
            ...
            action, _ = rula_action_level(grand)
            css_cls = "card-green" if grand <= 2 else "card-orange" if grand <= 4 else "card-red"
            rula_ph.markdown(
                f'<div class="risk-card {css_cls}"><b>RULA Grand Score: {grand}</b><br>'
                f'<small>{action}</small></div>', unsafe_allow_html=True)
        ...
        if reba_d:
            reba_ph.markdown(
                ...
                f'<small>{reba_d["reba_level"]}</small></div>', unsafe_allow_html=True)

        if ocra_d:
            css = "card-green" if ocra_d["ocra_level"] == "Acceptable" else \
                  "card-orange" if ocra_d["ocra_level"] == "Borderline" else "card-red"
            ocra_ph.markdown(
                f'<div class="risk-card {css}"><b>OCRA Index: {ocra_d["ocra_index"]}</b><br>'
                f'<small>{ocra_d["ocra_level"]}</small></div>', unsafe_allow_html=True)

        # ── Angle Table ──
        if angles:
            nice_names = {
                "R_upper_arm": "Right Upper Arm (°)", "R_elbow": "Right Elbow (°)",
                "R_wrist": "Right Wrist (°)",         "L_upper_arm": "Left Upper Arm (°)",
                "L_elbow": "Left Elbow (°)",          "L_wrist": "Left Wrist (°)",
                "neck": "Neck (°)",                   "trunk": "Trunk (°)",
                "R_knee": "Right Knee (°)",           "R_hip": "Right Hip (°)",
            }
        ...
        if hist and len(hist) % 5 == 0:
            ...
            if rula_d and reba_d:
                ...
                fig_heat = px.bar(df_heat, x="Region", y="Risk Score",
                                  color="Risk Score",
